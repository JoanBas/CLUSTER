...
class MODEL():
    def __init__(self, time=0, increased_duration_factor=1, event_kinds=['views', 'edits'],dataset_name="handson3"):
...
"""def make_session(usr_id,t):
    ...
    print usr_id,n
    if n==0:
        return [np.empty([0]),np.empty([0])], t
    alpha, beta, mu = in_sess[usr_id]
    session = simulate_multi_hawkes(alpha, beta, mu, n,reference_stop_event=reference_stop_event)
    session[0]+=t
    session[1]+=t
    ...
    for sec in session:
        try:
            ...
            val = float("-inf")
        T = max(T, val)
    print len(session[0]),len(session[1])
    return session, T
...
M=2
event_kinds=['comments','edits']
event_kind_sess_len_reference='edits'
reference_stop_event="none"#event_kinds.index(event_kind_sess_len_reference) #index of the event used to decid when to stop simulating an event
plotAll=0


##############################################

reader = csv.reader(open('Files/in_sess_multi_'+"_".join(str(x) for x in event_kinds)+'.csv', 'r'))
in_sess = {}
for row in reader:
    parameters=row[1:]
...
reader = csv.reader(open('Files/sess_len_dist_'+event_kind_sess_len_reference+'.csv', 'r'))
sess_len_dist = {}
for row in reader:
    sess_len_dist[row[0]] =  np.asarray(row[1:]).astype(int)

P = dataset(event_kinds=event_kinds)
users = [P.data[ke] for ke in event_kinds]
maxdate = datetime.strptime('9-11-2014 23:59:59', '%d-%m-%Y %H:%M:%S')
mindate = datetime.strptime('3/11/2014 00:00:00', '%d/%m/%Y %H:%M:%S')
users_timelines=[{},{}]

#Loop per triar linici de les sessions i omplir aquestes sessions
print in_sess.keys(), len(in_sess.keys())
print sess_dist.keys(), len(sess_dist.keys())
for usr_id in in_sess.keys():
    t0=0.
    ti=0.
    t=0.
    timeline=[np.empty([1,0]) for i in range(M)]
...
cdf([users_timelines[0],P.dates_to_minutes(pre_users=P.data['comments'])])
cdf([users_timelines[1],P.dates_to_minutes(pre_users=P.data['edits'])])"""
