...
def simulate_multi_hawkes(alpha, beta, mu, Nmax, plot=0):
    M=len(mu)
...
def find_values(x0):
    res = minimize(loglik, x0, method='nelder-mead', options={'xtol': 1e-3, 'disp': True, 'maxiter':10000})
    if res.success:
        params = res.x ** 2
...
def train_hawkes_parameters(k,combined=0):
    events_list = users[k]
    x0=np.random.rand(10)*2
    for i in range(len(events_list)-1,-1,-1):
        if len(events_list[i][0])<2: continue
        inici=float("inf")
        for events in events_list[i]:
            try:
                val = events[0]
            except:
                val = float("inf")
        ...
        xs.append( [np.asarray(events_list[i][0]) - inici, np.asarray(events_list[i][1]) - inici] )
...
if __name__ == "__main__":
    ...
    try:
        reader = csv.reader(open('Files/in_sess_multi2.csv', 'r'))
        parameters = {}
        for row in reader:
    ...
    file = open("Files/in_sess_multi2.csv", "wt")
    ...
    data = np.genfromtxt('Files/in_sess_multi2.csv', delimiter=',')
