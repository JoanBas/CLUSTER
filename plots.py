...
if __name__ == "__main__":
    ...
    data = np.genfromtxt('Files/in_sess_multi_comments_edits.csv', delimiter=',')
    print mean_std(data[:, 1:-1])
    exit()
    plot_1v1(data[:, 1] / data[:, 5], data[:, 5])
    print data
    exit()

    data = np.genfromtxt('Files/in_sess.csv', delimiter=',')
    ...
    if filter:
        for i in range(len(data)):
            if data[i][2]<5:
                normal_users.append(str(int(data[i][0])))
            else: radical_users.append(str(int(data[i][0])))
    ...
    ax.scatter(data[:,1],data[:,2],data[:,3],marker=".")
    ...
    P=dataset()
    usrs=P.prepare()
    for k in normal_users:
        alpha,beta,mu=data_dict[k]
        print k,";",data_dict[k]
        xs=simulate_uni_hawkes(alpha,beta,mu,50)
        x= [xs]+usrs[k][:10]
        plot_timelines(x)


    """P=prepara()
    P.modes(IDlist=radical_users)
    P.modes(IDlist=normal_users)"""